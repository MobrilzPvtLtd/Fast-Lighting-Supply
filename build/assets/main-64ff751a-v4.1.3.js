class d {
    constructor(e) {
        this.slidePanelHtml = this.getSlidePanelHtml(e);
    }
    getSlidePanelHtml(e) {
        e.slide.options = e.slide.options || this.getDefaultOptions();
        let t = _.template($("#slide-template").html());
        return $(t(e));
    }
    getDefaultOptions() {
        return {
...
class n {
    constructor() {
        (this.slideCount = 0),
            ...
            this.slideCount === 0 && this.addSlide(),
            this.attachEventListeners(),
            this.makeSlidesSortable();
    }
    addSlides(e) {
        for (let t of e) this.addSlide(t);
    }
    addSlide(e = {}) {
        let t = new d({ slideNumber: this.slideCount++, slide: e });
        $("#slides-wrapper").append(t.render());
    }
    attachEventListeners() {
        $(".add-slide").on("click", () => {
            this.addSlide();
